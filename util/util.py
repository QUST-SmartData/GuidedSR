...
def mkdirs(paths):  #递归创建路径
    """create empty directories if they don't exist

    Parameters:
        paths (str list) -- a list of directory paths  目录路径列表
    """
    if isinstance(paths, list) and not isinstance(paths, str):    #isinstabce(object,classinfo)判断是否为某一类型
        for path in paths:
            mkdir(path)
    else:
        mkdir(paths)
...
def mkdir(path):    #递归创建路径
    """create a single empty directory if it didn't exist

    Parameters:
        path (str) -- a single directory path   单个目录路径
    """

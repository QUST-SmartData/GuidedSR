...
if __name__ == '__main__':
    ...
    # Create dataset with fixed sample patches   使用固定样本补丁创建数据集
    opt_eval = TrainOptions().parse()
    opt_eval.num_train, opt_eval.num_test, opt_eval.eval_mode = 100, 100, False
    ...
    # Evaluate metrics before running the model    在运行模型之前评估度量
    metrics_log_file = model.save_dir + '/' + opt.name + '_metrics.txt'
    util.eval_error_metrics(0, model, dataset_eval, log_filename=metrics_log_file)  #输出PSNR和SSIM
